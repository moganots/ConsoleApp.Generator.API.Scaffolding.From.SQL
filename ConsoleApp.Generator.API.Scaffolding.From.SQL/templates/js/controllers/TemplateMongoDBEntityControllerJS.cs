...
{
    public class TemplateMongoDBEntityControllerJS
    {
        ...
        public TemplateMongoDBEntityControllerJS() { }
        public TemplateMongoDBEntityControllerJS(string author, string dateCreated, string schemaName, string entityName, string controllerDirectory)
        {
            Author = author;
        ...
        public void Generate()
        {
            ...
            sb.AppendLine($"| Description   : MongoDB Entity (Model) Controller for the [{SchemaName}].[{EntityName}] Table");
            sb.AppendLine("|--------------------------------------------------------------------------------------------------------------------------------------------");
            sb.AppendLine(" */");
            sb.AppendLine("");
            sb.AppendLine("/**");
            sb.AppendLine($" * Creates an instance of the [{SchemaName}].[{EntityName}] MongoDB Entity (Model) Controller");
            ...
            sb.AppendLine($" * @returns MongoDB{EntityName!.ProperCase()}Controller");
            sb.AppendLine(" */");
            ...
            sb.AppendLine($"      `MongoDB{EntityName!.ProperCase()}Controller - [logger] instance has not been provided or set`");
            sb.AppendLine("    );");
            sb.AppendLine("  }");
            sb.AppendLine("  if (notSet(dbContext)) {");
            sb.AppendLine("    throw new Error(");
            sb.AppendLine($"      `MongoDB{EntityName!.ProperCase()}Controller - [dbContext] instance has not been provided or set`");
            sb.AppendLine("    );");
            sb.AppendLine("  }");
            sb.AppendLine("  const dto =");
            sb.AppendLine($"    require(`./../../models/entities/MongoDB/{EntityName!.GetEntityFileName()}`)();");
            //sb.AppendLine("  const dbContext = require(`./../../middleware/database/context/MongoDB/db.context`)(");
            //sb.AppendLine("    logger,");
            //sb.AppendLine("    config");
            //sb.AppendLine("  );");
            sb.AppendLine("  const { getRequestQueryParametersWithoutUid } =");
            ...
            sb.AppendLine("        request.query.uid,");
            sb.AppendLine("        dto,");
            ...
            sb.AppendLine("    try {");
            sb.AppendLine("      dbContext.getAll(request.query.uid, dto, (error, data, message) => {");
            sb.AppendLine("        return log(request, response, error, data, message);");
            sb.AppendLine("      });");
            sb.AppendLine("    } catch (exception) {");
            sb.AppendLine("      return log(request, response, exception, null, null);");
